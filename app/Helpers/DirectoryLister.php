...
class DirectoryLister
{
	...
	public function __construct()
	{
		...
		$this->_config = config('cms.directory_lister');
		
		// Set the file types array to a global variable
		$this->_fileTypes = config('cms.directory_lister.file_types');
	}
	...
	protected function _setDirectoryPath($dir) 
	{
		...
		if (!file_exists($dir) || !is_dir($dir)) {
			// Set the error message
			$this->setSystemMessage('danger', '<strong>Error:</strong> File path does not exist');
		...
		if ($this->_isHidden($dir)) {
			// Set the error message
			$this->setSystemMessage('danger', '<strong>Error:</strong> Access denied');
				...
				if ($file == '..') {
					if ($this->_directory != '.') {
						...
						if (!empty($directoryPath)) {
							...
							if (strpos($directoryPath, 'cp') == false) {
								$directoryPath = $directoryPath.'cp/assets';
							}
						}
