...
class EmailLoginNotification extends Notification implements ShouldQueue
{
     ...
     * @return void
     */
    public function __construct(string $token, string $firstName, string $subject = 'Email Login')
    {
		$this->token = $token;
    ...
    public function toMail($notifiable)
    {
		...
		$data = [
			...
			'emailLoginUrl' => url('/login/email', $this->token)
		];
            
		return (new MailMessage)->view('emails.email.login', $data)->subject($this->subject);
	}
