							...
							<table class="text-muted table table-responsive" cellspacing="0" border="0" cellpadding="0" width="100%">
								...
								<tbody>
									@foreach ($order->product_commodities as $productCommodity)
										<tr>
											<td class="align-middle text-center"><a href="{{ $productCommodity->product->url }}" title="{{ $productCommodity->product->title }}" target="_blank" class="text-gf-info"><img data-src="{{ $productCommodity->product->image_url }}" class="lazyload img-fluid" width="100px" alt="{{ $productCommodity->product->title }}"></a></td>
