					...
					<form name="removeArticle" id="removeArticle" class="removeArticle" role="form" method="POST" action="/cp/articles/{{ $article->id }}">
						...
						<div class="form-buttons">
							@if ($currentUser->hasPermission('view_articles'))
								<a href="/cp/articles" title="Cancel" class="btn btn-link text-secondary" tabindex="2" title="Cancel">Cancel</a>
							@endif
							<button type="submit" name="submit_remove_article" id="submit_remove_article" class="pull-right btn btn-danger" tabindex="1" title="Delete">Delete</button>
						</div>
					</form>
				</div>
