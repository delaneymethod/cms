...
@section('title', 'Edit Location - Locations - '.config('app.name'))
@section('description', 'Edit Location - Locations - '.config('app.name'))
...
@section('formButtons')
	<div class="form-buttons">
		@if ($currentUser->hasPermission('view_locations'))
			<a href="/cp/locations" title="Cancel" class="btn btn-link text-secondary" tabindex="16" title="Cancel">Cancel</a>
		@endif
		<button type="submit" name="submit_edit_location" id="submit_edit_location" class="pull-right btn btn-primary" tabindex="15" title="Save Changes"><i class="icon fa fa-check-circle" aria-hidden="true"></i>Save Changes</button>
	</div>
@endsection
		...
		<div class="row">
			...
			<div class="{{ $mainSmCols }} {{ $mainMdCols }} {{ $mainLgCols }} {{ $mainXlCols }} main">
				...
				<div class="content padding bg-white">
					<form name="editLocation" id="editLocation" class="editLocation" role="form" method="POST" action="/cp/locations/{{ $location->id }}">
						{{ csrf_field() }}
						{{ method_field('PUT') }}
						<input type="hidden" name="solution_id" value="{{ $location->solution_id }}">
						@if (in_array($location->id, $defaultLocationIds))
							<input type="hidden" name="status_id" value="{{ $location->status_id }}">
						@endif
						@yield('formButtons')
						...
						<div class="form-group">
							...
							<select name="county_id" id="county_id" class="form-control" tabindex="10" aria-describedby="helpBlockCountyId" required>
								@foreach ($counties as $county)
									<option value="{{ $county->id }}" {{ (old('county_id') == $county->id || $location->county_id == $county->id) ? 'selected' : '' }}>{{ $county->title }}</option>
								@endforeach
							</select>
						...
						<div class="form-group">
							...
							<select name="country_id" id="country_id" class="form-control" tabindex="11" aria-describedby="helpBlockCountryId" required>
								@foreach ($countries as $country)
									<option value="{{ $country->id }}" {{ (old('country_id') == $country->id || $location->country_id == $country->id) ? 'selected' : '' }}>{{ $country->title }}</option>
								@endforeach
							</select>
						...
						<div class="form-group">
							...
							<select name="company_id" id="company_id" class="form-control" tabindex="13" aria-describedby="helpBlockCompanyId" required>
								@foreach ($companies as $company)
									<option value="{{ $company->id }}" {{ (old('company_id') == $company->id || $location->company_id == $company->id) ? 'selected' : '' }}>{{ $company->title }}</option>
								@endforeach
							</select>
						...
						<div class="form-group">
							...
							@endif
							@if (in_array($location->id, $defaultLocationIds))
								<span id="helpBlockStatusId" class="form-control-feedback form-text text-warning">- This field is disabled. The status cannot be changed for a default location.</span>
							@endif
